  ...
  useEffect(() => {
    ...
    const saveConsulta = async () => {
      ...
      try {
        ...
        // Preparar datos del paciente
        const { fullName, gender, ageYears, ageMonths, temperature, municipality, neighborhood, additionalInfo, conditions } = patientInfo;
        const otros_datos = JSON.stringify({ municipality, neighborhood, additionalInfo, conditions });
        const clasificacion = await clasificarDiagnostico(diagnosis.preliminaryDiagnosis.explanation);
        // Agregar municipio y colonia al JSON del diagnóstico
