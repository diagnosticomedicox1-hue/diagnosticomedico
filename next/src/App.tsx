...
// Inicializar Google Gemini con API key desde variables de entorno de Next
...
const AppContent = () => {
  ...
  const generateDiagnosis = async () => {
    ...
    if (!genAI) {
      setError('La clave NEXT_PUBLIC_GEMINI_API_KEY no está configurada en las variables de entorno.');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        diagnosisData.recommendations = diagnosisData.recommendations.map((rec: any) => ({
        ...
        diagnosisData.doctors = diagnosisData.doctors.map((doc: any) => ({
      ...
      <nav className="bg-blue-600 p-4 shadow-md">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex space-x-6">
            <Link to="/" className={`text-white font-medium hover:text-blue-100 transition-colors ${location.pathname === '/' ? 'underline underline-offset-4 decoration-2' : ''}`}>
              Prediagnóstico
            </Link>
            <Link to="/consulta" className={`text-white font-medium hover:text-blue-100 transition-colors ${location.pathname === '/consulta' ? 'underline underline-offset-4 decoration-2' : ''}`}>
              Consulta Georreferenciada
            </Link>
          </div>
          <div className="hidden sm:block text-blue-100 text-sm italic">
            Universisad de Colima - Medicina
          </div>
        </div>
      </nav>
      <Routes>
